...
Require Import FO_Bi_Int_remove_list.
...
pose (wFOBIC_subst _ (cycle_shift n) H). simpl in w.
pose (cycle_shift_subject n A H0). rewrite e in w. clear e.
pose (cycle_shift_shift n B). rewrite e in w.
...
destruct H3 ; subst. unfold In. exists x0 ; split ; auto. rewrite cycle_shift_shift ; auto.
apply H2. unfold In ; auto. unfold In. inversion H1. exists x0. destruct H3 ; subst. split ; auto.
rewrite cycle_shift_shift ; auto. apply H2. unfold In ; auto. rewrite H1 in w.
...
- simpl. unfold gen_choice_code. destruct (decoding _ _ _ _ (S n)).
...
Lemma der_nextension_theory_mextension_theory_le : forall n m (Γ Δ: @Ensemble (form)) A,
  (wFOBIC_rules (fst (nextension_theory Γ Δ n), A)) -> (le n m) ->
    (wFOBIC_rules  (fst (nextension_theory Γ Δ m), A)).
Proof.
intros.
pose (wFOBIC_monot (fst (nextension_theory Γ Δ n), A) H (fst (nextension_theory Γ Δ m))).
simpl in w. apply w. intro. intros. apply nextension_theory_extens_le with (n:=n) ; auto.
Qed.
...
destruct x.
- simpl in H2. pose (wFOBIC_monot (fst (nextension_theory Γ Δ n), ⊥) H2 (fst (nextension_theory Γ Δ m))).
  simpl in w. exists []. repeat split ; auto. simpl. apply w. intro. intros.
...
- simpl in H2. simpl in H1. destruct x. simpl in H2. apply absorp_Or1 in H2.
  ...
  pose (wFOBIC_monot (fst (nextension_theory Γ Δ n), f) H2 (fst (nextension_theory Γ Δ m))).
  apply w. simpl. intro. intros. apply nextension_theory_extens_le with (n:=n) ; auto.
...
apply wFOBIC_Deduction_Theorem with (A0:=A) (B:= list_disj x) (Γ0:=Γ) in H2 ; auto.
pose (Id_list_disj_remove Γ (remove eq_dec_form A x0) x).
pose (list_disj_remove_form _ _ A H4).
assert (J1: wFOBIC_rules (Γ, list_disj (remove eq_dec_form A x0) --> list_disj (remove eq_dec_form A x0 ++ remove_list (remove eq_dec_form A x0) x))).
apply Id_list_disj.
...
- simpl in H0. apply IHn with (Γ:=Γ) (Δ:=Δ) ; auto. unfold gen_choice_code in H0.
  remember (decoding _ _ _ _ (S n)) as code. destruct code.
  + destruct f ; auto ; destruct H0 ; destruct H0 ; destruct H1.
    (* Then in each case check if either adding to the left or the right preserve derivability. *)
    ...
    * simpl in H2. 
      ...
      apply (wFOBIC_Deduction_Theorem _ H7) ; auto. inversion H9. subst. apply wTop. inversion H10.
      ...
      apply (wFOBIC_Deduction_Theorem _ H6) ; auto. inversion H8. subst. apply wTop. inversion H9.
      ...
      apply (wFOBIC_Deduction_Theorem _ H2) ; auto. inversion H4. subst. apply wTop. inversion H5.
    ...
    * destruct q.
          ...
          { assert ((fun x : form => In form (fst (nextension_theory Γ Δ n)) x \/
            ...
            - destruct (classic (exists n0 : nat, First_unused n0
              ...
              + destruct H6. destruct H6. exists (remove eq_dec_form f[$x1..] x). repeat split ; auto. apply remove_preserv_NoDup ; auto.
                 ...
                 assert (wFOBIC_rules (fst (nextension_theory Γ Δ n), f[$x1..] ∨ list_disj (remove eq_dec_form f[$x1..] x))).
                 ...
                 subst. auto. inversion H10. apply comm_Or in H8. pose (wFOBIC_finite _ H8). destruct e. destruct H9.
                 destruct p. destruct e. pose (finite_context_list_conj _ _ w0 x3 H9). simpl in w1.
                 assert (J1: wFOBIC_rules (Singleton form (list_conj x3), list_disj (remove eq_dec_form f[$x1..] x) ∨ list_disj (f[$x1..] :: List.nil))).
                 ...
                 pose (wFOBIC_Dual_Deduction_Theorem (list_conj x3) (list_disj (remove eq_dec_form f[$x1..] x)) (f[$x1..] :: List.nil) J1).
                 simpl w2. apply absorp_Or1 in w2.
                 ...
                 pose (Gen_unused x1 (Singleton form (list_conj x3 --< list_disj (remove eq_dec_form f[$x1..] x))) f J2 w2).
                 assert (J3: wFOBIC_rules (Singleton form (list_conj x3 --< list_disj (remove eq_dec_form f[$x1..] x)), (∀ f) ∨ ⊥)).
                 ...
                 pose (wFOBIC_Dual_Detachment_Theorem (list_conj x3) (list_disj (remove eq_dec_form f[$x1..] x)) (∀ f :: List.nil) J3).
                 simpl in w4.
                 assert (wFOBIC_rules (Singleton form (list_conj x3), list_disj (remove eq_dec_form f[$x1..] x) ∨ (∀ f))).
                 ...
                 apply wFOBIC_monot with (Γ1:=fst (nextension_theory Γ Δ n)) in w5. simpl in w5.
                 apply comm_Or in w5. apply list_disj_In in w5. auto. apply in_in_remove ; auto. intro.
            ...
            - destruct (classic (exists n0 : nat, First_unused n0
              ...
              + destruct H6. destruct H6. exists ((∀ f) :: remove eq_dec_form f[$x1..] x). repeat split ; auto. apply NoDup_cons.
                 ...
                 (fst (nextension_theory Γ Δ n), list_disj x)]). 2: apply MPRule_I. intros. inversion H8. subst. auto. inversion H9.
                 subst. auto. inversion H10. apply comm_Or in H8. pose (wFOBIC_finite _ H8). destruct e. destruct H9.
                 destruct p. destruct e. pose (finite_context_list_conj _ _ w0 x3 H9). apply comm_Or. simpl in w1.
                 assert (J1: wFOBIC_rules (Singleton form (list_conj x3), list_disj (remove eq_dec_form f[$x1..] x) ∨ list_disj (f[$x1..] :: List.nil))).
                 simpl. apply MP with (ps:=[(Singleton form (list_conj x3), (list_disj (remove eq_dec_form f[$x1..] x) ∨ f[$x1..]) --> (list_disj (remove eq_dec_form f[$x1..] x) ∨ (f[$x1..] ∨ ⊥)));
                 (Singleton form (list_conj x3), list_disj (remove eq_dec_form f[$x1..] x) ∨ f[$x1..])]). 2: apply MPRule_I.
                 intros. inversion H10. subst.
                 ...
                 inversion H14. 2: inversion H15. subst. apply wEFQ. inversion H13. inversion H11. subst. 2: inversion H12. auto.
                 pose (list_conj_finite_context x3 x2 (list_disj (remove eq_dec_form f[$x1..] x) ∨ (∀ f)) H9 H10).
                 apply wFOBIC_monot with (Γ1:=fst (nextension_theory Γ Δ n)) in w5. simpl in w5. auto.
      ...
      -- simpl in H2. simpl in H1.
          ...
          { assert (exists n0 : nat, First_unused n0 (fun y : form => In form (fst (nextension_theory Γ Δ n)) y \/ In form (snd (nextension_theory Γ Δ n)) y \/ y = ∃ f)).
            ...
            apply wFOBIC_Deduction_Theorem with (A:=f[$x0..]) (B:=list_disj x)
            (Γ0:=(Union form (fst (nextension_theory Γ Δ n)) (Singleton form (∃ f)))) in H2 ; auto.
            assert (J1: wFOBIC_rules (Union form (fst (nextension_theory Γ Δ n)) (Singleton form (∃ f)), list_disj x)).
...
Lemma encode_In_L_or_R : forall (A : form) (Γ Δ: @Ensemble (form)),
  ...
  (closed_S Δ) ->
  (wpair_der (Γ, Δ) -> False) ->
  (In form (fst (nextension_theory Γ Δ (encode _ _ _ _ A))) A \/ In form (snd (nextension_theory Γ Δ (encode _ _ _ _ A))) A).
Proof.
intros. simpl. unfold gen_choice_code.
remember (encode _ _ _ _ A) as n.
assert (S (encode0 _ _ _ _ A) = encode _ _ _ _ A). auto. rewrite H2. clear H2.
...
- simpl. left. unfold In. right. repeat split ; auto. intros.
  apply (Under_nextension_theory (encode0 _ _ _ _ ⊤) _ _ H H0 H1).
  assert (wpair_der ((fst (nextension_theory Γ Δ (encode0 _ _ _ _ ⊤))) , Union form (snd (nextension_theory Γ Δ (encode0 _ _ _ _ ⊤))) (Singleton form ⊤))).
  ...
  simpl. apply MP with (ps:=[(fst (nextension_theory Γ Δ (encode0 _ _ _ _ ⊤)), ⊤ --> (⊤ ∨ ⊥));(fst (nextension_theory Γ Δ (encode0 _ _ _ _ ⊤)), ⊤)]).
...
- simpl. destruct (classic (wpair_der (Union form (fst (nextension_theory Γ Δ (encode0 _ _ _ _ (atom P t)))) (Singleton form (atom P t)), snd (nextension_theory Γ Δ (encode0 _ _ _ _ (atom P t)))))).
  right. unfold In. right. repeat split ; auto. unfold In. left. right. split ; auto.
- simpl. destruct (classic (wpair_der (Union form (fst (nextension_theory Γ Δ (encode0 _ _ _ _ (bin b A1 A2)))) (Singleton form (bin b A1 A2)), snd (nextension_theory Γ Δ (encode0 _ _ _ _ (bin b A1 A2)))))).
  right. unfold In. right. repeat split ; auto. left. unfold In. right. auto.
- simpl. destruct q.
  + simpl. destruct (classic (wpair_der (Union form (fst (nextension_theory Γ Δ (encode0 _ _ _ _ (∀ A)))) (Singleton form (∀ A)), snd (nextension_theory Γ Δ (encode0 _ _ _ _ (∀ A)))))).
     right. unfold In. right. repeat split ; auto. left. unfold In. right ; auto.
  + simpl. destruct (classic (wpair_der (Union form (fst (nextension_theory Γ Δ (encode0 _ _ _ _ (∃ A)))) (Singleton form (∃ A)), snd (nextension_theory Γ Δ (encode0 _ _ _ _ (∃ A)))))).
...
pose (nextension_theory_extens_le (Init.Nat.max x (encode _ _ _ _ A0)) (encode _ _ _ _ A0) Γ Δ A0).
...
apply (Under_nextension_theory (encode _ _ _ _ A) _ _ H H0 H1). exists [A].
repeat split. apply NoDup_cons. intro ; inversion H5. apply NoDup_nil.
intros. inversion H5 ; subst. 2: inversion H6.
pose (nextension_theory_extens_le (Init.Nat.max x (encode _ _ _ _ A0)) x Γ Δ A0).
destruct a. apply H6 in H4 ; try lia. rewrite e in H4 ; auto. simpl (list_disj (A :: List.nil)).
apply MP with (ps:=[(fst (nextension_theory Γ Δ (encode _ _ _ _ A)), A --> (A ∨ ⊥)); (fst (nextension_theory Γ Δ (encode _ _ _ _ A)), A)]).
2: apply MPRule_I. intros. inversion H5. subst. apply Ax. apply AxRule_I.
apply RA2_I. exists A. exists ⊥. auto. inversion H6. 2: inversion H7. subst.
apply Id. apply IdRule_I.
pose (nextension_theory_extens_le (encode _ _ _ _ A) x Γ Δ A).
...
Lemma In_extension_In_encode_R : forall (A : form) (Γ Δ: @Ensemble (form)),
  ...
  In form (snd (nextension_theory Γ Δ (encode _ _ _ _ A))) A.
Proof.
intros. pose (encode_In_L_or_R A _ _ H H0 H1). destruct o ; auto.
exfalso. unfold extension_theory in H2. simpl in H2. inversion H2.
pose (Nat.max_dec x (encode _ _ _ _ A)). destruct s.
...
apply RA2_I. exists A. exists ⊥. auto. inversion H6. 2: inversion H7. subst.
apply Id. apply IdRule_I.
pose (nextension_theory_extens_le x (encode _ _ _ _ A) Γ Δ A).
destruct a. apply H7 ; try lia ; auto.
apply (Under_nextension_theory (encode _ _ _ _ A) _ _ H H0 H1). exists [A].
repeat split. apply NoDup_cons. intro ; inversion H5. apply NoDup_nil.
intros. inversion H5 ; subst. 2: inversion H6.
pose (nextension_theory_extens_le (encode _ _ _ _ A0) x Γ Δ A0).
destruct a. apply H7 ; try lia ; auto.
apply MP with (ps:=[(fst (nextension_theory Γ Δ (encode _ _ _ _ A)), A --> (A ∨ ⊥)); (fst (nextension_theory Γ Δ (encode _ _ _ _ A)), A)]).
2: apply MPRule_I. intros. inversion H5. subst. apply Ax. apply AxRule_I.
apply RA2_I. exists A. exists ⊥. auto. inversion H6. 2: inversion H7. subst.
...
Lemma max_list_encode : forall l, (exists m, forall n, (exists A, encode _ _ _ _ A = n /\ List.In A l) -> n <= m).
Proof.
induction l.
- exists 0. intros. destruct H. destruct H. inversion H0.
- destruct IHl. exists (Nat.max (encode _ _ _ _ a) x). intros. destruct H0. destruct H0.
  inversion H1. subst. lia. subst.
  assert (exists A : form, encode _ _ _ _ A = encode _ _ _ _ x0 /\ List.In A l). exists x0 ; auto.
  pose (H (encode _ _ _ _ x0) H0). lia.
Qed.
...
simpl in H2. pose (wFOBIC_finite _ H2). destruct e. simpl in H3. destruct H3. destruct p. destruct e.
assert (exists ml, forall n, (exists A, encode _ _ _ _ A = n /\ List.In A x1) -> n <= ml).
apply max_list_encode. destruct H4.
assert (exists mr, forall n, (exists A, encode _ _ _ _ A = n /\ List.In A x) -> n <= mr).
apply max_list_encode. destruct H5.
pose (Under_nextension_theory (max x2 x3) _ _ ClΓ ClΔ H). apply f.
...
pose (nextension_theory_extens_le (Init.Nat.max x2 x3) (encode _ _ _ _ A) Γ Δ A).
destruct a. apply H9 ; auto. assert (exists A0 : form, encode _ _ _ _ A0 = encode _ _ _ _ A /\ List.In A0 x).
exists A ; auto. pose (H5 (encode _ _ _ _ A) H10). lia.
apply (wFOBIC_monot _ w). simpl. intro. intro.
pose (In_extension_In_encode_L x4 _ _ ClΓ ClΔ H).
pose (i _ H6). pose (H3 x4). destruct p. pose (i2 H6).
pose (nextension_theory_extens_le (Init.Nat.max x2 x3) (encode _ _ _ _ x4) Γ Δ x4).
destruct a. apply H7 ; auto. assert (exists A : form, encode _ _ _ _ A = encode _ _ _ _ x4 /\ List.In A x1).
exists x4 ; auto. pose (H4 (encode _ _ _ _ x4) H9). lia.
Qed.
...
destruct (classic (wpair_der ((Union _ (fst (nextension_theory Γ Δ (encode0 _ _ _ _ A))) (Singleton _ A)), snd (nextension_theory Γ Δ (encode0 _ _ _ _ A))))).
- right. exists (encode _ _ _ _ A). simpl. unfold gen_choice_code. remember (decoding _ _ _ _ (S (encode0 _ _ _ _ A))) as o.
  destruct o.
  + assert (S (encode0 _ _ _ _ A) = encode _ _ _ _ A). auto. rewrite H1 in Heqo. rewrite encode_decode_Id in Heqo. inversion Heqo. subst.
      ...
      * simpl. exfalso. apply (Under_extension_theory _ _ ClΓ ClΔ H).
         ...
         apply MP with (ps:=[(fst (nextension_theory Γ Δ (encode0 _ _ _ _ ⊤)), ⊤ --> (⊤ ∨ ⊥));(fst (nextension_theory Γ Δ (encode0 _ _ _ _ ⊤)), ⊤)]).
         ...
         intros. apply H3 in H5. pose (nextension_theory_extens (encode0 _ _ _ _ ⊤) Γ Δ A). destruct a.
        apply (extension_theory_extens_nextension (encode0 _ _ _ _ ⊤)) ; auto.
        apply (wFOBIC_monot _ H4). intro. apply (extension_theory_extens_nextension (encode0 _ _ _ _ ⊤)) ; auto.
      * simpl. right. repeat split ; auto.
      * simpl. right. repeat split ; auto.
      * destruct q.
        -- simpl. right. repeat split ; auto.
        -- simpl. right. repeat split ; auto.
  + exfalso. assert (S (encode0 _ _ _ _ A) = encode _ _ _ _ A). auto. rewrite H1 in Heqo. rewrite encode_decode_Id in Heqo. inversion Heqo.
- left. exists (encode _ _ _ _ A). simpl. unfold gen_choice_code. remember (decoding _ _ _ _ (S (encode0 _ _ _ _ A))) as o.
  destruct o.
  + assert (S (encode0 _ _ _ _ A) = encode _ _ _ _ A). auto. rewrite H1 in Heqo. rewrite encode_decode_Id in Heqo. inversion Heqo. subst.
...
(* Prime theories are closed under derivations. *)

Lemma closeder_fst_Lind_pair : forall (Γ Δ: @Ensemble (form)),
  (closed_S Γ) ->
  (closed_S Δ) ->
  (wpair_der (Γ, Δ) -> False) ->
  (forall A, wFOBIC_rules (fst (extension_theory Γ Δ), A) -> (In _ (fst (extension_theory Γ Δ)) A)).
Proof.
...
Lemma Lwitness_Ex_help : forall A (Γ Δ: @Ensemble (form)),
  ...
  (wpair_der (Γ, Δ) -> False) ->
  In form (fst (extension_theory Γ Δ)) (∃ A) ->
  (exists n, In form (fst (nextension_theory Γ Δ (encode _ _ _ _ (∃ A)))) A[(var n)..]).
Proof.
intros.
pose (In_extension_In_encode_L (∃ A) _ _ H H0 H1 H2).
simpl. unfold gen_choice_code.
assert (S (encode0 _ _ _ _ (∃ A)) = encode _ _ _ _ (∃ A)). auto. rewrite H3. clear H3.
rewrite encode_decode_Id. simpl.
assert (Unused: exists n0 : nat, First_unused n0 (fun y : form => In form (fst (nextension_theory Γ Δ (encode0 _ _ _ _ (∃ A)))) y \/ In form (snd (nextension_theory Γ Δ (encode0 _ _ _ _ (∃ A)))) y \/ y = ∃ A)).
pose (nextension_infinite_unused (encode0 _ _ _ _ (∃ A)) _ _ H H0 H1).
assert (J0: (exists n : nat, unused_S n (fun y : form => In form (fst (nextension_theory Γ Δ (encode0 _ _ _ _ (∃ A)))) y \/ In form (snd (nextension_theory Γ Δ (encode0 _ _ _ _ (∃ A)))) y)
/\ (forall m : nat, n <= m -> unused_S m (fun y : form => In form (fst (nextension_theory Γ Δ (encode0 _ _ _ _ (∃ A)))) y \/ In form (snd (nextension_theory Γ Δ (encode0 _ _ _ _ (∃ A)))) y)))).
...
assert ((Union form (fun y : form => In form (fst (nextension_theory Γ Δ (encode0 _ _ _ _ (∃ A)))) y \/ In form (snd (nextension_theory Γ Δ (encode0 _ _ _ _ (∃ A)))) y)
(fun x : form => List.In x (∃ A :: List.nil))) = (fun y : form => In form (fst (nextension_theory Γ Δ (encode0 _ _ _ _ (∃ A)))) y \/ In form (snd (nextension_theory Γ Δ (encode0 _ _ _ _ (∃ A)))) y \/ y = ∃ A)).
...
apply (Under_nextension_theory (encode _ _ _ _ (∃ A)) _ _ H H0 H1).
exists x0. repeat split ; auto. intros. apply H5 in H7.
assert (encode0 _ _ _ _ (∃ A) <= encode _ _ _ _ (∃ A)). unfold encode. lia.
pose (nextension_theory_extens_le (encode _ _ _ _ (∃ A)) (encode0 _ _ _ _ (∃ A)) Γ Δ A0). destruct a.
apply H10 ; auto.
apply MP with (ps:=[(fst (nextension_theory Γ Δ (encode _ _ _ _ (∃ A))), (∃ A) --> list_disj x0);(fst (nextension_theory Γ Δ (encode _ _ _ _ (∃ A))), ∃ A)]).
2: apply MPRule_I. intros. inversion H7. subst.
apply (wFOBIC_Deduction_Theorem (Union form (fst (nextension_theory Γ Δ (encode _ _ _ _ (∃ A)))) (Singleton form (∃ A)), list_disj x0)) ; auto.
apply (wFOBIC_monot _ H6) ; auto. intro. intros. inversion H8 ; subst. apply Union_introl.
apply (nextension_theory_extens_le (encode _ _ _ _ (∃ A)) (encode0 _ _ _ _ (∃ A)) Γ Δ x1) ; auto. unfold encode ; auto.
...
assert (exists n, In form (fst (nextension_theory Γ Δ (encode _ _ _ _ (∃ A)))) A[(var n)..]).
...
Lemma Rwitness_All_help : forall A (Γ Δ: @Ensemble (form)),
  ...
  (wpair_der (Γ, Δ) -> False) ->
  In form (snd (extension_theory Γ Δ)) (∀ A) ->
  (exists n, In form (snd (nextension_theory Γ Δ (encode _ _ _ _ (∀ A)))) A[(var n)..]).
Proof.
intros.
...
destruct Unused. exists x. unfold In. right. split.
destruct (classic (wpair_der (Union form (fst (nextension_theory Γ Δ (encode0 _ _ _ _ (∀ A)))) (Singleton form (∀ A)), snd (nextension_theory Γ Δ (encode0 _ _ _ _ (∀ A)))))) ; auto.
exfalso. apply (Under_nextension_theory (encode _ _ _ _ (∀ A)) _ _ H H0 H1). exists [∀ A].
...
apply MP with (ps:=[(fst (gen_choice_code (fst (nextension_theory Γ Δ (encode0 _ _ _ _ (∀ A)))) (snd (nextension_theory Γ Δ (encode0 _ _ _ _ (∀ A)))) (S (encode0 _ _ _ _ (∀ A)))), (∀ A) --> (∀ A) ∨ ⊥);
(fst (gen_choice_code (fst (nextension_theory Γ Δ (encode0 _ _ _ _ (∀ A)))) (snd (nextension_theory Γ Δ (encode0 _ _ _ _ (∀ A)))) (S (encode0 _ _ _ _ (∀ A)))), (∀ A))]).
...
clear H6. clear H5. unfold gen_choice_code. assert (S (encode0 _ _ _ _ (∀ A)) = encode _ _ _ _ (∀ A)). auto.
...
assert (exists n, In form (snd (nextension_theory Γ Δ (encode _ _ _ _ (∀ A)))) A[(var n)..]).
