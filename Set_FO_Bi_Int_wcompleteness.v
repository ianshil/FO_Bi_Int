...
Instance universal_interp : interp term :=
    {| i_func := func ; i_atom := fun _ _ => False |}.
...
(* A1 --> A2 *)
  * split ; intro.
      +  destruct (classic (In form e0 (A1 --> A2))).
          ...
          intro. apply H0. apply gen_wFOBIC_Deduction_Theorem in H1.
          { apply clos_deriv with (Δ:=e1). unfold ExAllCompNotDer ; auto. auto. }
          admit.

          (* Need to weaken the Lindenbaum lemma? Maybe I can simply  ask that the theory has
              some unused variables.

              The problem here is that there is no unused variable in e0, which does not allow to
              apply the Lindenbaum lemma on the pair in H1. Do I need to introduce a new set of
              constants / variables in the Lindenbaum lemma? Or find another way to prove this
              point here? Not sure at all. *)

          (* apply Lindenbaum_lemma in H1.
          destruct H1. destruct x. simpl in H1. destruct H1.
          destruct H2. destruct H3. destruct H4. destruct H5.
          assert (J1: ExAllCompNotDer (e, e2)). unfold ExAllCompNotDer ; auto.
          pose (@exist (prod (Ensemble form) (Ensemble form)) ExAllCompNotDer (e,e2) J1).
          pose (H s).
          assert (J2: Canon_relation (exist (fun x : Ensemble form * Ensemble form =>
          ExAllCompNotDer x) (e0, e1) (conj w (conj w0 (conj c f)))) s). unfold Canon_relation. simpl.
          intro. intros. apply H1. apply Union_introl. auto. apply k in J2.
          pose (IH (size A2)). apply i in J2 ; simpl ; try lia ; auto. clear i. simpl in J2.
          apply H6. exists [A2]. repeat split. apply NoDup_cons ; auto ; apply NoDup_nil.
          intros. inversion H7. subst. simpl. apply H2. apply In_singleton. inversion H8. simpl.
          simpl. apply MP with (ps:=[(e, A2 --> (A2 ∨ ⊥));(e, A2)]). 2: apply MPRule_I.
          intros. inversion H7. subst. apply Ax. apply AxRule_I. apply RA2_I. exists A2. exists ⊥.
          auto. inversion H8. 2: inversion H9. subst. apply Id. apply IdRule_I. auto.
          pose (IH (size A1)). apply i ; simpl ; try lia ; auto. clear i.
          simpl. apply H1. apply Union_intror. apply In_singleton. auto.
          (* The last two goals are not true. *) all: admit. *)
  ...
  * split ; intro.
   + destruct H. destruct H. destruct (classic (In form e0 (A1 --< A2))). auto.
    ...
    + assert (wpair_der ((Singleton _ A1), Union _ e1 (Singleton _ A2)) -> False).
      ...
      assert (wFOBIC_rules (Singleton _ A1, A2 ∨ (list_disj (remove eq_dec_form A2 x)))).
      ...
      auto. inversion H9. clear w. clear H2.
      ...
      pose (wFOBIC_Deduction_Theorem (Union _ (Empty_set _) (Singleton _ A1),
      A2 ∨ (list_disj (remove eq_dec_form A2 x))) H7 A1 (A2 ∨ (list_disj (remove eq_dec_form A2 x)))
      (Empty_set _) J1 J2). apply wdual_residuation in w. clear J2. clear J1. clear H2. clear H5.
      ...
      pose (wFOBIC_monot (Empty_set _, (A1 --< A2) --> list_disj (remove eq_dec_form A2 x))
      w e0). apply w0. clear w0. simpl. intro. intros. inversion H5. inversion H5.
      ...
      (* apply Lindenbaum_lemma in H0. destruct H0. destruct x. simpl in H0. destruct H0. destruct H1.
      destruct H2. destruct H3. destruct H4.
      assert (J1: ExAllCompNotDer (e2, e3)). unfold ExAllCompNotDer. auto.
      pose (@exist (prod (Ensemble _) (Ensemble _)) ExAllCompNotDer (e2,e3) J1).
      exists s. unfold Canon_relation. simpl. repeat split.
      pose (IH (size A1)). apply i ; simpl ; try lia ; auto. clear i. apply H0. apply In_singleton.
      intro. intros. unfold ExAllCompNotDer in e. simpl in e. destruct e. destruct H8. destruct H9.
      pose (H9 x). destruct o.
      simpl in H11. auto. simpl in H11. exfalso. pose (H1 x). simpl in i.
      assert (In form (Union form e1 (Singleton form A2)) x).
      apply Union_introl. auto. apply i in H12. apply H5. exists [x].
      repeat split. apply NoDup_cons ; auto ; apply NoDup_nil.
      intros. inversion H13. subst. simpl. auto. inversion H14. simpl.
      apply MP with (ps:=[(e2, x --> (x ∨ ⊥)); (e2, x)]).
      2: apply MPRule_I. intros. inversion H13. subst.
      apply Ax. apply AxRule_I. apply RA2_I. exists x. exists ⊥. auto.
      inversion H14. 2: inversion H15. subst. apply Id. apply IdRule_I. auto.
      intro. pose (IH (size A2)). apply i in H6 ; simpl ; try lia ; auto. clear i.
      simpl in H6. apply H5. exists [A2].
      repeat split. apply NoDup_cons ; auto ; apply NoDup_nil.
      intros. inversion H7. subst. simpl. apply H1. apply Union_intror.
      apply In_singleton. inversion H8. simpl.
      apply MP with (ps:=[(e2, A2 --> (A2 ∨ ⊥));(e2, A2)]). 2: apply MPRule_I.
      intros. inversion H7. subst. apply Ax. apply AxRule_I. apply RA2_I. exists A2.
      exists ⊥. auto. inversion H8. 2: inversion H9. subst. apply Id. apply IdRule_I. auto. auto.
       (* The last two goals are not true. *) all: admit. *)
(* Quantifiers *)
- destruct q.
  (* Universal *)
...
(* Depending on the Lindenbaum lemma (LL) I can get, I may need to talk about
    pairs of lists and not pairs of sets:
      - if LL is with enswpair_der then I can have completeness for pairs of sets
      - if LL is with wpair_der then I can have completeness for pairs of lists.

    UPDATE: the distinctions lists / finite or enumerable sets / infinite sets only matters
    w.r.t. constructivity. So, given that I use classical logic, I don't care. Let's go with all sets.

    Another element to consider is the type of theories I consider: it may be easier
    to start with lists (resp. sets) of closed formulae, and then see if I can extend my
    results to open formulae. Closed formulae are easier to handle as I can make
    the lists (resp. sets) Henkin and dual-Henkin without worrying about the variables
    I pick in the addition of Henkin and dual-Henkin formulae to the initial list (resp. set).

    UPDATE: we go with closed sets first. It is doable to do it with sets of open formulae
    but it is more technical to play the trick of finding the right unused variable. *)
...
apply Lindenbaum_lemma with (list_Funcs0:=list_Funcs) (list_Preds0:=list_Preds)
in WD ; auto. destruct WD. destruct x. simpl in H. destruct H. destruct H0.
...
Theorem wCompleteness : forall (Γ Δ : @Ensemble form),
    ...
    (wpair_der (Γ, Δ)).
Proof.
intros Γ Δ ClΓ ClΔ LC. pose (wCounterCompleteness Γ Δ).
